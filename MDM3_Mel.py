...
def create_mmi( params=None):
    ...
    if params["resonator_type"] == "fish":
        ...
        ext_ext_up = c.add_ref(gf.components.straight(length=1.4, width=10, layer=(1, 0)))
        ...
        ext_ext_down = c.add_ref(gf.components.straight(length=1.4, width=10, layer=(1, 0)))
...
def create_resonator_or_smw(component_type: str, taper_length: float = 10, taper_width1: float = 0.08,
    ...
    text1 = c.add_ref(gf.components.text(text=str(resonator), size=2)).dmovex(28).dmovey(-4+y_spacing).flatten()
...
def run_coupon_mode(base_directory, today_date, clearance_width,to_debug,layers):
    ...
    if not to_debug:
        c.add_ref(gf.components.straight(length=85, width=400,layer=layers["coarse_ebl_layer"])).dmovex(-83).dmovey(400/2-30).dmovex(
            -clearance_width).flatten()
        c.add_ref(gf.components.straight(length=10, width=500,layer=layers["coarse_ebl_layer"])).dmovex(-95).dmovey(500/2-80).dmovex(
            -clearance_width).flatten()
